...
public:
    ...
    {
        out << " Date: " << t.date << " Timestamp: " << t.timestamp << " Open: " << t.open << " High: "<<t.high<<" Low: "<<t.low<<" Close: "<<t.close
...
public:
    Stock() :symbol("")
    {
        dailyTrades.clear(); intradayTrades.clear();
    }
    ...
    ~Stock() {}
    void addDailyTrade(const DailyTrade& aTrade)
    {
        dailyTrades.push_back(aTrade);
    }
    void addIntradayTrade(const IntradayTrade& aTrade)
    {
        intradayTrades.push_back(aTrade);
    }
    string GetSymbol() const { return symbol;}
    const vector<DailyTrade>& GetDailyTrade(void) const
    {
        return dailyTrades;
    }
    const vector<IntradayTrade>& GetIntradayTrade(void) const
    {
